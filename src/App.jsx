...
export default function App() {
 ...
 async function bookDay(day) {
   ...
   if (existing === selFam) {
     ...
     if (supabase) {
       ...
       showToast("Reserved! (offline mode)");
     }
   }
 }
 ...
 async function addNote() {
   ...
   const tempNote = {id:Date.now(),text:t,author:selFam,timestamp:fmtNow(),resolved:false};
   ...
   if (supabase) {
     ...
     if(data) setNotes(n=>n.map(x=>x.id===tempNote.id?{...x,id:data.id}:x));
     setSaving(false);
   }
 ...
 async function toggleNote(id, resolved) {
   ...
   if (supabase) await supabase.from("notes").update({resolved:!resolved}).eq("id",id);
 }
 ...
 return (
   <div style={{minHeight:"100vh",background:BG,fontFamily:"'EB Garamond','Georgia',serif",color:INK,margin:0,padding:0,maxWidth:600,marginLeft:"auto",marginRight:"auto"}}>
     ...
     {/* DB status bar - only shows if there's an issue */}
     {dbStatus === "error" && (
       <div style={{background:"#8B4040",color:"#F5EDE0",padding:"6px 12px",textAlign:"center",fontFamily:"'Josefin Sans',sans-serif",fontSize:9,letterSpacing:"2px",textTransform:"uppercase"}}>
         Database connection error — bookings won't save
       </div>
     )}
     {dbStatus === "offline" && (
       <div style={{background:"#7A5C2E",color:"#F5EDE0",padding:"6px 12px",textAlign:"center",fontFamily:"'Josefin Sans',sans-serif",fontSize:9,letterSpacing:"2px",textTransform:"uppercase"}}>
     ...
     <div style={{padding:"16px 12px 48px"}}>
       ...
       {tab==="calendar" && (
         <>
           {loading && (
             <div style={{textAlign:"center",padding:"20px 0",fontFamily:"'Josefin Sans',sans-serif",fontSize:9,letterSpacing:"2px",textTransform:"uppercase",color:LIGHT}}>Loading…</div>
           )}
       ...
       {tab==="notes" && (
         <div>
           ...
           {openNotes.length===0&&<div style={{textAlign:"center",padding:"36px 0",fontFamily:"'EB Garamond',serif",fontStyle:"italic",fontSize:16,color:LIGHT}}>The house is in good order.</div>}
           {openNotes.length>0&&(
             <>
